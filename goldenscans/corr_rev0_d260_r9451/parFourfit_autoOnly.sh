#!/bin/bash
#
# parFourfit_autoOnly.sh <fourfit control file> <scan1> [<scan2> <scan3> ...]
#
# Fourfit of autocorrelations only, including cross-polar autos.
#
# Runs several fourfit instances in parallel, per baseline, for each
# of the given scan(s). The scripts waits if more than 20 fourfit instances
	...
	for bl in "${blines[@]}"; do
		if compgen -G "./$indir/$bl..*" > /dev/null; then
			...
			if compgen -G "./$indir/$bl.[A-Z].*" > /dev/null; then
				...
				fourfit -m1 -c $1 -b$bl $FF_EXTRA_ARGS -u $indir $FF_POST_ARGS 2>&1 | grep SNR &
				err=$?
				if [  "$err" != "0" ]; then
